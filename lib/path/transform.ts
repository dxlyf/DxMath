...
import { SCALAR_NEARLY_ZERO } from "./scalar"
function mul_add_mul(a: f32, b: f32, c: f32, d: f32):f32 {
 ...
 function compute_inv(ts: Transform, inv_det: f64):Transform {
    return Transform.from_row(
...
function dcross(a: f64, b: f64, c: f64, d: f64):f64 {
    return a * b - c * d
}
function dcross_dscale(a: f32, b: f32, c: f32, d: f32, scale: f64):f32 {
    return (dcross(a as f64, b as f64, c as f64, d as f64) * scale) as f32
}
 const inv_determinant=(ts:Transform):number|undefined=>{
    let det:f32=dcross(ts.sx, ts.sy, ts.kx, ts.ky) 
    let tolerance=  SCALAR_NEARLY_ZERO * SCALAR_NEARLY_ZERO * SCALAR_NEARLY_ZERO;
    if(Num.new(det).is_nearly_zero_within_tolerance(tolerance)){
        return undefined
    }else{
        return 1/det
    }
   ...
   static from_rotate_at(angle:number,tx:number,ty:number):Transform{
    ...
    is_valid(){
        if(this.is_finite()){
            ...
            let sx=Num.new(scale.x)
            let sy=Num.new(scale.y)
            return !()
        }else{
            return false
        }
    ...
    get_scale(){
        let self=this;
        let x_scale = Num.new(self.sx*self.sx + self.kx * self.kx).sqrt().value;
        let y_scale = Num.new(self.ky*self.ky + self.sy * self.sy).sqrt().value;
        return Point.create(x_scale, y_scale)
    }
