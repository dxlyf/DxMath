...
const FLOAT_PI: f32 =Math.PI;
export type LineProc = (points:Point[],_ScreenIntRect:Option<ScreenIntRect>,_Blitter:Blitter)=>void
...
function hair_line_rgn(points:Point[], clip: Option<ScreenIntRect>, blitter:Blitter) {
    let max = 32767.0;
    let fixed_bounds = Rect.from_ltrb(-max, -max, max, max).unwrap();

    let clip_bounds = clip?.clone()

    for(let i=0,len=points.length-1;i<len;i++) {
        let  pts = [Point.zero(),Point.zero()];

        // We have to pre-clip the line to fit in a Fixed, so we just chop the line.
        if (!line_clipper.intersect([points[i], points[i + 1]], fixed_bounds,pts)) {
            continue;
        }

        if(clip_bounds!==undefined) {
            let tmp = pts.clone();
            // Perform a clip in scalar space, so we catch huge values which might
            // be missed after we convert to FDot6 (overflow).
            if (!line_clipper.intersect(tmp,clip_bounds,pts)) {
                continue;
            }
        }
