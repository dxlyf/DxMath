...
import { SK_Scalar1, SK_ScalarNearlyZero, SkDegreesToRadians, SkDoubleToScalar, SkScalarAbs, SkScalarCosSnapToZero, SkScalarHalf, SkScalarInvert, SkScalarIsFinite, SkScalarNearlyEqual, SkScalarNearlyZero, SkScalarSinSnapToZero, SkScalarSqrt, SkScalarSquare } from "./scalar";
...
function sk_ieee_float_divide(numer, denom) {
    return numer / denom;
}
function checkForZero(x) {
    return x * x == 0;
}
function sk_ieee_double_divide(numer, denom) {
    return numer / denom;
}
function only_scale_and_translate(mask) {
    return 0 == (mask & (kAffine_Mask | kPerspective_Mask));
}
function is_degenerate_2x2(scaleX, skewX,
    skewY, scaleY) {
    let perp_dot = scaleX * scaleY - skewX * skewY;
    return SkScalarNearlyZero(perp_dot, SK_ScalarNearlyZero * SK_ScalarNearlyZero);
}
function SkSignBitTo2sCompliment(x) {
    if (x < 0) {
        x &= 0x7FFFFFFF;
        x = -x;
    }
...
function SkScalarAs2sCompliment(v) {
    return v
}
function SkFloat2Bits(x) {
...
function muladdmul(a, b, c, d) {
    return (a * b + c * d);
}

function rowcol3(row, col) {
...
export class Matrix {
    ...
    */
    static RotateDeg(deg, pt?: Point) {
        let m = this.identity();
        m.setRotate(deg, pt);
        return m;
    }

    static RotateRad(rad) {
    ...
    */
    static MakeAll(scaleX, skewX, transX,
        skewY, scaleY, transY,
        pers0, pers1, pers2) {
        let m = this.identity();
    ...
    */
    getScaleX() { return this.fMat[Matrix.kMScaleX]; }
    ...
    */
    setScaleX(v) { return this.set(kMScaleX, v); }

    /** Sets vertical scale factor.
    
        @param v  vertical scale factor to store
    */
    setScaleY(v) { return this.set(kMScaleY, v); }
    ...
    */
    setPolyToPoly(src: Point[], dst: Point[], count) {
        if (count > 4) {
    ...
    */
    mapPoints(dst: Point[], src: Point[], count) {
    ...
    */
    mapHomogeneousPoints(dst: Point[], src: Point[], count) {
    ...
    */
    mapVectors(dst: Point[], src: Point[], count) {
        if (this.hasPerspective()) {
    ...
    */
    mapRect(dst, src, pc = SkApplyPerspectiveClip.kYes) {
