...
import {PathVerbAnalysis} from './path_ref'
import {PathVerb,PathSegmentMask, PathDirection} from './path_types'
import {Rect} from './rect'
import {RRect} from './rrect'
function sk_path_analyze_verbs(vbs:number[],verbCount:number) {
    let info =PathVerbAnalysis.make(false, 0, 0, 0);
    ...
    for (let i = 0; i < verbCount; ++i) {
        switch (vbs[i]) {
            ...
            case PathVerb.kLine:
                invalid |= needMove;
                info.segmentMask |= PathSegmentMask.kLine_SkPathSegmentMask;
                info.points += 1;
                break;
            case PathVerb.kQuad:
                invalid |= needMove;
                info.segmentMask |= PathSegmentMask.kQuad_SkPathSegmentMask;
                info.points += 2;
                break;
            case PathVerb.kConic:
                invalid |= needMove;
                info.segmentMask |= PathSegmentMask.kConic_SkPathSegmentMask;
                info.points += 2;
                info.weights += 1;
                break;
            case PathVerb.kCubic:
                invalid |= needMove;
                info.segmentMask |= PathSegmentMask.kCubic_SkPathSegmentMask;
                info.points += 3;
                break;
...
/**\类SKPATH
    SKPath包含几何形状。 SKPath可能是空的，或包含一个或多个动词
    概述一个数字。 SKPATH总是从移动动词到笛卡尔坐标开始，
    然后可能是增加线或曲线的其他动词。
    添加一个近距离动词使几何形状成为连续的循环，一个封闭的轮廓。
    SKPATH可能包含任何数量的轮廓，每个轮廓都以移动动词开头。
SKPATH轮廓可能仅包含一个移动动词，或者可能包含线条，
    二次贝齐斯，圆锥形和立方体。 SKPATH轮廓可能开放或
    关闭。

    当用于绘制填充区域时，SKPath描述了填充物是内部还是
    在几何形状之外。 SKPath还描述了用于填充的绕组规则
    重叠轮廓。

    在内部，SKPATH懒惰计算指标喜欢界限和凸度。称呼
SKPATH :: UPDATEBOUNDSCACHE使SKPath线程安全。
*/
export class  Path {
    static default(){
        return new this()
     ...
     */
    static Make(pts:Point[],pointCount:number,
          vbs:number, verbCount:number,
        ws:number,wCount:number,
         ft:PathFillType, isVolatile:boolean) {
        if (verbCount <= 0) {
            return Path.default();
        }

        const  info = sk_path_analyze_verbs(vbs, verbCount);
        if (!info.valid || info.points > pointCount || info.weights > wCount) {
       
         return Path.default();
        }

        return Path.MakeInternal(info, pts, vbs, verbCount, ws, ft, isVolatile);
    }

    static Rect(r:Rect,dir:PathDirection=PathDirection.kCW,startIndex:number=0) {
                        return PathBuilder().addRect(r, dir, startIndex).detach();
     }
...
public:
    ...
    */
    enum Verb {
        kMove_Verb  = static_cast<int>(PathVerb.kMove),
        kLine_Verb  = static_cast<int>(PathVerb.kLine),
        kQuad_Verb  = static_cast<int>(PathVerb.kQuad),
        kConic_Verb = static_cast<int>(PathVerb.kConic),
        kCubic_Verb = static_cast<int>(PathVerb.kCubic),
        kClose_Verb = static_cast<int>(PathVerb.kClose),
    ...
    private:
        constexpr static int pts_advance_after_verb(SkPathVerb verb) {
            switch (verb) {
                ...
                case PathVerb.kQuad: return 2;
                case PathVerb.kConic: return 2;
                case PathVerb.kCubic: return 3;
                case PathVerb.kClose: return 0;
            }
            SkUNREACHABLE;
        }
        constexpr static int pts_backset_for_verb(SkPathVerb verb) {
            switch (verb) {
                case PathVerb.kMove: return 0;
                case PathVerb.kLine: return -1;
                case PathVerb.kQuad: return -1;
                case PathVerb.kConic: return -1;
                case PathVerb.kCubic: return -1;
                case PathVerb.kClose: return -1;
            }
            SkUNREACHABLE;
        }
