...
//int26_6是签名的26.6定点号。
//
//整数零件范围为-33554432至33554431，包括。这
//分数部分具有6位精确度。
//
//例如，数字一个和四分之一是int26_6（1 << 6 + 1 << 4）。
export class Int26_6{
   ...
   toString()  {
        const shift=6, mask=1<<6 - 1
        if (this.value >= 0) {
            //return console.log("%d:%02d", int32(x>>shift), int32(x&mask))
        }
    ...
    floot(){
        return Math.trunc((this.value+0x00)>>6)
    }
    round(){
        return Math.trunc((this.value+0x20)>>6)
    }
    ceil(){
        return Math.trunc((this.value+0x3f)>>6)
    }
    mul(y:Int26_6){
        return new Int26_6(Number((BigInt(this.value)*BigInt(y.value)+BigInt(1<<5)) >> 6n)>>0)
    }
}
