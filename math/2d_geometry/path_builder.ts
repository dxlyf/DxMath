...
export function getQuadraticBounds(p0: Point, p1: Point, p2: Point) {
    let tValues = [0, 1];
...
export function getCubicBounds(p0: Point, p1: Point, p2: Point, p3: Point) {
...
export function quadraticBezierSubdivide(p0: Point, p1: Point, p2: Point, t: number = 0.5) {
...
// 点与线段距离
export function pointOnSegmentDistance(pt: Point, a: Point, b: Point) {
...
export function buildPathFromSvgPath(svgPath: string, path: PathBuilder) {
...
type PathVisitor = {
    moveTo: (record: { type: PathVerb, p0: Point }) => void
    lineTo: (record: { type: PathVerb, p0: Point }) => void
    quadraticCurveTo: (record: { type: PathVerb, p0: Point, p1: Point, p2: Point }) => void
...
export class PathBuilder {
    ...
    addPath(path: PathBuilder, matrix?: number[]) {
        ...
        this.lastMoveIndex = Math.min(this.points.length - 1, path.lastMoveIndex + this.points.length)
        this.needMoveVerb = path.needMoveVerb
