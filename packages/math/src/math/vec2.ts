...
export class Vector2 {
    ...
    */
    ...
    // 按数学y轴,上是正，下是负，但网页中，上是页，下是正，顺时针的话，y为正
    // 按网页坐标轴计算，如果想按数学使用rotateCW
    cw(){
        return this.set(-this.y,this.x)
    }
    ccw() {
    ...
    interpolatesVectors(start: Vector2, end: Vector2, t: Vector2) {
        return this.set(start.x + (end.x - start.x) * t.x, start.y + (end.y - start.y) * t.y)
    }
    ...
    lerp(start: Vector2, end: Vector2, t: number) {
        return this.mix(start, end, t)
    }
    swap() {
        return this.set(this.y, this.x)
    }
    ...
    applyTransform(a:number,b:number,c:number,d:number,e:number,f:number) {
        return this.set(a * this.x + c * this.y + e,
            b * this.x + d * this.y + f)
    }
