...
import {Bezier} from "../bezier-js/bezier";
...
export abstract class Seg {
  ...
  print() {
    return _.map(this.points, (point:Point) => point.print()).join(" ");
  }
  ...
  extendIntersects(seg: Seg): Point|null {
  ...
  toSvg(isFirst: boolean): string {
    const svg = `${this.command} ${_.map(_.tail(this.points), (point:Point) => `${point.x},${point.y}`).join(" ")}`;
...
const getQuadraticBezier = (firstPoint: Point, centerPoint: Point, lastPoint: Point,): QuadraticBezier => {
  const points = Bezier.quadraticFromPoints(
    firstPoint,
    centerPoint,
    lastPoint,
    0.5,
...
export class Path {
  ...
  static connectPatchWhenNeed = (segs: Seg[], isClosed: boolean, linejoin: StrokeLinejoins): Seg[] => {
      ...
      if (prevSeg && !prevSeg.getLastPoint().eql(seg.getFirstPoint())) {
        ...
        if (connection) {
          ...
          if (prevSeg instanceof Line) {
            ...
            const curve = new Bezier(prevSeg.points);
          ...
          if (seg instanceof Line) {
            ...
            const curve = new Bezier(seg.points);
    ...
    const result = _.reduce(segs, (finalSegs:Seg[], seg: Seg) => {
      const [needToOverwritePrev, connectSegs] = getPatchedSegs(_.last(finalSegs), seg);
    ...
    if (isClosed) {
      ...
      if (!firstSeg.getLastPoint().eql(lastSeg.getFirstPoint())) {
        ...
        if (needToOverwritePrev) {
          return ([] as any[]).concat(connectSegs).concat(otherSegs);
        }
  ...
  print() {
    return _.map(this.segs, (seg:Seg) => seg.print()).join(" | ");
  }
  ...
  offset(radius: number) {
    return _.reduce(
      this.segs,
      (finalPath:Path, seg:Seg) => finalPath.connect(seg.offset(radius), false),
      new Path([], false, this.linejoin),
    );
  }

  toSvg(): string {
    return _.map(this.segs, (seg:Seg, idx:number, segs:Seg[]) => seg.toSvg(idx === 0)).join("") + (this.closed ? "Z" : "");
  }
}