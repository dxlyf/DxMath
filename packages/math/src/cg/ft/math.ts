 ...
 * 缩放因子，用于伪旋转校正
 * tan(theta)=2^-i
 * 1/Math.sqrt(1+Math.pow(2,-1*2))=cos=arctan(tan(theta))
 * function computeCordicGain(iterations: number): number {
  let k = 1.0;
  let k2=1.
  for (let i = 0; i < 23; i++) {
    k *= 1 / Math.sqrt(1 + Math.pow(2, -2 * i)); // 0.6072529350088871
	k2*=1 / Math.sqrt(1 + Math.pow(2, -2 * (i+1))) // 8587853364804297 =k*sqrt(2)=0.8587853364804359
  }
 ...
 */
 ...
 /**
  * new Array(23).fill(0).reduce((d,v,i)=>d*Math.cos(Math.atan(Math.pow(2,-(i+1)))),1)=0.8587853364804297
 */
 export const FT_TRIG_SCALE=0xDBD95B16 // 32.32位固定点数
...
export const  FT_TRIG_MAX_ITERS=23
 ...
 // new Array(23).fill(0).reduce((d,v,i)=>d+Math.atan(Math.pow(2,-(i)))/Math.PI*180,0) [0-99]的角度范围
 // new Array(23).fill(0).reduce((d,v,i)=>d+Math.atan(Math.pow(2,-(i+1)))/Math.PI*180,0) [0,54]的角度范围
// 下列采用[0-54]
// 1740967=Math.atan(Math.pow(2,-1))/Math.PI*180*65536
...
// 旋转公式: r*cos(theta+alpha) = r*cos(theta) * cos(alpha) - r*sin(theta) * sin(alpha)=x * cos(alpha) - y * sin(alpha)=cos(alpha)*(x-y*tan(alpha))
// 单位圆
export function ft_trig_pseudo_rotate(vec:FT_Vector,theta:FT_Angle)
{
	...
	{
		// b:提升精度的常量.
		//**因为右移除法 >> i 的精度随着 i 增大而降低，**所以为了保持“加法偏移量”b的有效性，我们让它也成比例增长（即每轮 b <<= 1）。
		let v1:FT_Fixed = Number((BigInt(y) + b) >> BigInt(i)); //  y*tan(theta)=y*2^-i=y>>2^i
