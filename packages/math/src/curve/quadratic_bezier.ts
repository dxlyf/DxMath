...
/**
 * 二次贝塞尔曲线
     * 曲线方程：B(t) = (1-t)^2*P0 + 2*(1-t)*t*P1 + t^2*P2
     * 其中 P0,P1,P2 是曲线上的三个点，t 为参数（取值范围为 [0,1]）
     *
 */
export class QuadraticBezierCurve {
    ...
    /** 
     * 曲线的导数表示的是某点的切线方向，或者说该点处变化率（斜率）
    */
    derivativeAt(t:number){
        const a_x=2*(this.p1.x-this.p0.x)
        const a_y=2*(this.p1.x-this.p0.x)
        const b_x=2*t*(this.p2.x-2*this.p1.x+this.p0.x)
        const b_y=2*t*(this.p2.y-2*this.p1.y+this.p0.y)
        return Vector2.create(a_x+b_x, a_y+b_y)
    }
    getExtrema(){
      const a_x=(this.p1.x-this.p0.x)
      const a_y=(this.p1.x-this.p0.x)
      const b_x=(this.p2.x-2*this.p1.x+this.p0.x)
      const b_y=(this.p2.y-2*this.p1.y+this.p0.y)
      const t_x=-a_x/b_x
      const t_y=-a_y/b_y
      const roots=[0,1,t_x,t_y].filter(x=>Number.isFinite(x)&&x>=0&&x<=1)
      return roots
    }
    /***
     * 
    */
    getMaxCurvatureAt(t:number){
    ...
    */
    getMaxCurvature() {
        ...
        const A = Vector2.create(p1.x - p0.x,p1.y - p0.y);
        const D =  Vector2.create(p2.x - 2 * p1.x + p0.x, p2.y - 2 * p1.y + p0.y);

  
        // 若 D 退化为零向量，则曲线退化为直线，曲率恒为 0
        if (D.lengthSq() === 0) return 0;

        // 求最优 t（极值）
        let t = -A.dot(D)/ D.lengthSq();
        // 限制 t 落在 [0,1]
        if (t < 0) t = 0;
        if (t > 1) t = 1;
        return t;
 ...
 */
     ...
     * @param t 参数 t
     */
    derivativeAt2(t: number): Vector2 {
