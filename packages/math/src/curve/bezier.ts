...
type Point = Vector2
type IPoint={x:number,y:number}|Vector2
...
export function getTOnCubicBezier(p0: Point, p1: Point, p2: Point, p3: Point, pt: Vector2, tolerance = 1e-6) {
...
type AA={
    [K in keyof Vector2]:Vector2[K]
}
type BB=keyof Pick<Vector2,'x'|'y'>

// 给定控制点 P0, P1, P2 和目标点 target，求二次贝塞曲线上使得 P[coord] = target[coord] 的 t 值
export function getTQuadratic(P0:Vector2, P1:Vector2, P2:Vector2, target:Vector2, coord:'x'|'y'= 'x') {
    // 二次曲线在该坐标上的表达式展开后：
...
export function getTCubic(P0:Vector2, P1:Vector2, P2:Vector2, P3:Vector2, target:Vector2, coord:'x'|'y' = 'x') {
    // 三次曲线的系数
...
export function solveQuadratic(a:number, b:number, c:number, epsilon = 1e-5) {
    const roots: number[] = [];
    ...
    // 检查根是否在 [0,1] 范围内
    const checkRoot = (t:number) => t >= -epsilon && t <= 1 + epsilon;
    if (checkRoot(t1)) {
...
// 主函数：求三次贝塞尔曲线极值点
export function findCubicBezierExtremes(P0:Vector2, P1:Vector2, P2:Vector2, P3:Vector2) {
    // ==================== X 方向极值 ====================
...
export const bezierDerivative = (pts: IPoint[], t: number) => {
    const n = pts.length - 1;
    let ret = { x: 0, y: 0 }
    for (let i = 0; i < n; i++) {
...
// 贝塞尔曲线二阶导数
export const bezierSecondDerivative = (pts: IPoint[], t: number) => {
    const n = pts.length - 1;
    let ret = { x: 0, y: 0 }
    for (let i = 0; i < n - 1; i++) {
...
一阶
如:p0,p1,p2 
c1=2*(p1-p0),c2=2*(p2-p1)
一阶导数：

c1(1-t)+c2t=2p1-2p0-2tp1+2tp0+2tp2-2tp1=2(p1-p0)+2t(p2-2p1+p0)=x
// 求最大极值，设置2(p1-p0)+2t(p2-2p1+p0)=0，解方程=0
t=-2(p1-p0)/2(p2-2p1+p0)=-(p1-p0)/(p2-2p1+p0) 最大极值t
求x导数
x=2(p1.x-p0.x)+2t(p2.x-2p1.x+p0.x)

二阶
q2=(c2-c1)=2*((p2-p1)-(p1-p0))=2*(p2-2*p1+p0)
*/
 ...
 */
export function newtonInterpolation(xValues:number[], yValues:number[], x:number) {
 ...
 */
export function rationalQuadraticToBeziers(P0:IPoint, P1:IPoint, P2:IPoint, w0:number, w1:number, w2:number, tolerance = 1e-6, maxDepth = 8):IPoint[][] {
 ...
 */
function subdivideRationalCurve(P0: IPoint, P1: IPoint, P2: IPoint, w0: number, w1: number, w2: number, t: number) {
    // 转换为齐次坐标
    ...
    ] as [[IPoint,IPoint,IPoint,number,number,number],[IPoint,IPoint,IPoint,number,number,number]];
}
...
/** 检查权重是否可转换为普通曲线 */
function areWeightsConvertible(w0:number, w1:number, w2:number, tolerance:number) {
    // 检查权重是否近似相等
 ...
 * 计算三次贝塞尔曲线在参数 t 处的坐标
 */
function bezierPoint(t:number, P0:IPoint, P1:IPoint, P2:IPoint, P3:IPoint) {
    const t1 = 1 - t;
    return [
        t1 ** 3 * P0.x + 3 * t1 ** 2 * t * P1.x + 3 * t1 * t ** 2 * P2.x + t ** 3 * P3.x,
        t1 ** 3 * P0.y + 3 * t1 ** 2 * t * P1.y + 3 * t1 * t ** 2 * P2.y + t ** 3 * P3.y
    ];
}
 ...
 * @returns {boolean}
 */
export function isPointOnBezier(Q:number[], P0:IPoint, P1:IPoint, P2:IPoint, P3:IPoint, epsilon = 2) {
    let t = 0.5; // 初始猜测参数
    let delta = Infinity;
    ...
    for (let i = 0; i < 10 && Math.abs(delta) > 1e-6; i++) {
        ...
        const dB = bezierDerivative([P0, P1, P2, P3],t);
        const d2B = bezierSecondDerivative([P0, P1, P2, P3],t);

        // 计算 f(t) 和 f'(t)
        const dx = B[0] - Q[0], dy = B[1] - Q[1];
        const f = dx * dB.x + dy * dB.y; // f(t) = (B - Q) · dB/dt
        const df = (dB.x ** 2 + dB.y ** 2) + (dx * d2B.x + dy * d2B.y); // f'(t)
...
export function cubicBezierPoint(P0:IPoint, P1:IPoint, P2:IPoint, P3:IPoint,t:number) {
    const u = 1 - t;
    const tt = t * t;
    const uu = u * u;
    const uuu = uu * u;
  ...
  export function cubicBezierDerivative(P0:IPoint, P1:IPoint, P2:IPoint, P3:IPoint,t:number) {
    const u = 1 - t;
    return {
  ...
  export function cubicBezierSecondDerivative(P0:IPoint, P1:IPoint, P2:IPoint, P3:IPoint,t:number) {
    const u = 1 - t;
    return {
  ...
  export function getClosestTOnCubicBezier_Newton(P0:IPoint, P1:IPoint, P2:IPoint, P3:IPoint, P:IPoint, tolerance = 1e-6, maxSteps = 20) {
    // 初值，先用采样法
  ...
  export function getClosestTOnCubicBezier2(p0:IPoint, p1:IPoint, p2:IPoint, p3:IPoint, p:IPoint, steps = 100, tolerance = 1e-6) {
